...
const MessagePipeline = struct {
    ...
    pub fn processMessage(self: *Self, json_str: []const u8, writer: anytype) !void {
        ...
        var extraction_result = self.content_extractor.extractFromJsonString(json_str) catch {
            // If extraction fails, return error instead of handling gracefully
...
pub fn main() !void {
    ...
    while (stream_reader.readLine() catch |err| switch (err) {
        ...
        pipeline.processMessage(line, stdout) catch |err| {
            ...
            // Write helpful error message to stderr
            switch (err) {
                error.JsonParseError => {
                    try std.io.getStdErr().writer().print("Error: Invalid JSON input\n", .{});
                },
                else => {
                    // Don't exit on other processing errors, just log and continue
...
test "MessagePipeline.init creates pipeline components" {
    ...
    // Enable colors explicitly for testing (since stdout isn't TTY in tests)
...
test "MessagePipeline.processMessage returns error for malformed JSON" {
    var pipeline = try MessagePipeline.init(testing.allocator);
    defer pipeline.deinit();
    ...
    // Should return error for malformed JSON (not handle gracefully)
    const result = pipeline.processMessage(invalid_json, output.writer());
    try testing.expectError(error.JsonParseError, result);

    // Output should be empty since error was returned
...
test "MessagePipeline.processMessage handles missing content with fallback" {
    ...
    const json_input =
        \\{"type":"status","timestamp":"2024-01-01T00:00:00Z"}
    ;
    ...
    // Should show metadata fallback since there's no content or data field
...
test "MessagePipeline memory management with multiple messages" {
    ...
    // Fix: Escape JSON curly braces to avoid format string confusion
    const json_template =
...
test "JSON template format string compilation" {
    // This test specifically verifies that JSON templates with curly braces
    // can be used safely with std.fmt.allocPrint by properly escaping braces

    // The fixed approach: double curly braces escape them in format strings
    const json_template =
        \\{{"type":"text","message":{{"content":"Test message {d}"}}}}
    ;

    // This should compile and execute without format string errors
    const test_json = try std.fmt.allocPrint(testing.allocator, json_template, .{42});
    defer testing.allocator.free(test_json);

    // Verify the output contains the expected content
    try testing.expect(std.mem.indexOf(u8, test_json, "\"type\":\"text\"") != null);
    try testing.expect(std.mem.indexOf(u8, test_json, "Test message 42") != null);
    try testing.expect(std.mem.indexOf(u8, test_json, "{\"type\":") != null); // Should start with single brace
}
